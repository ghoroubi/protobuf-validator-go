...
const (
	// RegWarning ;Warning message , is used in the regex validator generator method.
	RegWarning = "WARNING: regex and uuid validator is set for field %v.%v," +
		" only one of them can be set. Regex and UUIDRegEx validator is ignored for this field."

	// Warning message , is used in the proto2 validator generator method.
	Proto2Warning = "WARNING: field %v.%v is a proto2 message, validator.msg_exists has no effect\n"

	RepeatWarningMinELTS = "WARNING: field %v.%v is not repeated, validator.min_elts has no effects\n"

	RepeatWarningMaxELTS = "WARNING: field %v.%v is not repeated, validator.max_elts has no effects\n"

	RepeatWarningMsgExist = "WARNING: field %v.%v is repeated, validator.msg_exists has no effect\n"

	RepeatWarningNullable = "WARNING: field %v.%v is a nullable=false, validator.msg_exists has no effect\n"

	FloatWarning = "WARNING: field %v.%v has no 'float_lt' or 'float_gt' field so setting 'float_epsilon' has no effect."

	FloatWarningBoth = "WARNING: field %v.%v has both 'float_lt' and 'float_lte' constraints, only the strictest will be used."
)
...
// NewPlugin creates a new plugin and returns the interface which is implemented by.
...
// Init initiates the Generator of the plugin.
func (p *plugin) Init(g *generator.Generator) {
	p.Generator = g
}

// Generate generates the golang code of the provided file descriptor.
...
// generates and returns the field validator.
...
// generates an returns the one_of validator.
	...
	for _, field := range message.Field {
		...
		if v != nil {
			...
			if v.Regex != nil && v.UuidVer != nil {
				_, _ = fmt.Fprintf(os.Stderr, RegWarning, ccTypeName, fieldName)
				...
				if err != nil {
					...
					v.Regex = &uuid
					p.P(`var `, p.regexName(ccTypeName, fieldName),
						` = `, p.regexPkg.Use(),
						`.MustCompile(`, "`",
						*v.Regex, "`", `)`)
				}
...
//GetOneOfFieldName , returns the oneOf field name.
...
func (p *plugin) generateProto2Message(file *generator.FileDescriptor, message *generator.Descriptor) {
	...
	for _, field := range message.Field {
		...
		if p.validatorWithMessageExists(fieldValidator) {
			_, _ = fmt.Fprintf(os.Stderr, Proto2Warning, ccTypeName, fieldName)
		}
		...
		if repeated {
			...
			if field.IsMessage() || p.validatorWithNonRepeatedConstraint(fieldValidator) {
				p.P(`for _,i  := range `, variableName, `{`)
				p.In()
				variableName = "i"
			}
		} else if nullable {
		...
		if !repeated && fieldValidator != nil {
			if fieldValidator.RepeatedCountMin != nil {
				_, _ = fmt.Fprintf(os.Stderr, RepeatWarningMinELTS,
					ccTypeName, fieldName)
			}
			if fieldValidator.RepeatedCountMax != nil {
				_, _ = fmt.Fprintf(os.Stderr,
					RepeatWarningMaxELTS, ccTypeName, fieldName)
			}
		}
...
func (p *plugin) generateProto3Message(file *generator.FileDescriptor, message *generator.Descriptor) {
	...
	for _, oneOf := range message.OneofDecl {
		...
		if oneOfValidator.GetRequired() {
			...
			p.P(`return `,
				p.validatorPkg.Use(),
				`.FieldError("`, oneOfName, `",`,
				p.fmtPkg.Use(),
				`.Errorf("one of the fields must be set"))`)
			p.Out()
			p.P(`}`)
		}
	}
	for _, field := range message.Field {
		...
		nullable := (gogoproto.IsNullable(field) ||
			!gogoproto.ImportsGoGoProto(file.FileDescriptorProto)) &&
			field.IsMessage() &&
			!(p.useGogoImport && gogoproto.IsEmbed(field))
		...
		if repeated {
			...
			if field.IsMessage() ||
				p.validatorWithNonRepeatedConstraint(fieldValidator) {
				p.P(`for _, i := range `, variableName, `{`)
				p.In()
				variableName = "i"
			}
			...
			if fieldValidator.RepeatedCountMin != nil {
				_, _ = fmt.Fprintf(os.Stderr,
					RepeatWarningMinELTS, ccTypeName, fieldName)
			}
		...
		if field.IsString() {
			...
			if p.validatorWithMessageExists(fieldValidator) {
				if nullable && !repeated {
					...
					p.P(`return `,
						p.validatorPkg.Use(),
						`.FieldError("`, fieldName, `",`, p.fmtPkg.Use(),
						`.Errorf("message must exist"))`)
					p.Out()
					p.P(`}`)
				} else if repeated {
					_, _ = fmt.Fprintf(os.Stderr, RepeatWarningMsgExist,
						ccTypeName, fieldName)
				} else if !nullable {
					_, _ = fmt.Fprintf(os.Stderr, RepeatWarningNullable, ccTypeName, fieldName)
				}
			}
			if nullable {
				p.P(`if `, variableName, ` != nil {`)
			...
			p.P(`if err := `,
				p.validatorPkg.Use(),
				`.CallValidatorIfExists(`, variableName, `); err != nil {`)
			p.In()
			p.P(`return `,
				p.validatorPkg.Use(),
...
func (p *plugin) generateIntValidator(variableName string, ccTypeName string,
	fieldName string, fv *validator.FieldValidator) {
	if fv.IntGt != nil {
...
func (p *plugin) generateEnumValidator(
	...
	if fv.GetIsInEnum() {
		...
		p.P(`if _, ok := `,
			strings.Join(enum.TypeName(), "_"),
			"_name[int32(", variableName, ")]; !ok {")
		p.In()
		p.generateErrorString(variableName, fieldName,
			fmt.Sprintf("be a valid %s field", strings.Join(enum.TypeName(), "_")), fv)
		p.Out()
		p.P(`}`)
	}
}

func (p *plugin) generateLengthValidator(variableName string, ccTypeName string,
	fieldName string, fv *validator.FieldValidator) {
	if fv.LengthGt != nil {
...
func (p *plugin) generateFloatValidator(variableName string, ccTypeName string, fieldName string, fv *validator.FieldValidator) {
	...
	if fv.FloatEpsilon != nil && fv.FloatLt == nil && fv.FloatGt == nil {
		_, _ = fmt.Fprintf(os.Stderr, FloatWarning,
			ccTypeName, fieldName)
	}
	if fv.FloatLt != nil && fv.FloatLte != nil {
		_, _ = fmt.Fprintf(os.Stderr, FloatWarningBoth,
			ccTypeName, fieldName)
	...
	if fv.FloatGt != nil && fv.FloatGte != nil {
		_, _ = fmt.Fprintf(os.Stderr,
			"WARNING: field %v.%v has both 'float_gt' and 'float_gte' constraints, only the strictest will be used.",
			ccTypeName, fieldName)
...
// getUUIDRegex returns a regex to validate that a string is in UUIDRegEx
// format. The version parameter specified the UUIDRegEx version. If version is 0,
// the returned regex is valid for any UUIDRegEx version
func getUUIDRegex(version *int32) (string, error) {
	if version == nil {
		return "", nil
	} else if *version < 0 ||
		*version > 5 {
		return "", fmt.Errorf("UUIDRegEx version should be between 0-5, Got %d", *version)
	} else if *version == 0 {
...
func (p *plugin) generateRepeatedCountValidator(variableName string, ccTypeName string,
	fieldName string, fv *validator.FieldValidator) {
	if fv == nil {
		return
	}
...
func (p *plugin) generateErrorString(variableName string, fieldName string,
	specificError string, fv *validator.FieldValidator) {
	if fv.GetHumanError() == "" {
		p.P(`return `,
			p.validatorPkg.Use(),
			`.FieldError("`, fieldName, `",`, p.fmtPkg.Use(),
			".Errorf(`value '%v' must ",
			specificError, "`", `, `, variableName, `))`)
	} else {
		p.P(`return `,
			p.validatorPkg.Use(),
			`.FieldError("`, fieldName, `",`, p.fmtPkg.Use(), ".Errorf(`", fv.GetHumanError(), "`))")
	}
}

func (p *plugin) fieldIsProto3Map(file *generator.FileDescriptor,
	message *generator.Descriptor, field *descriptor.FieldDescriptorProto) bool {
